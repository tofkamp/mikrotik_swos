...
class Mikrotik_Port(Swostab):
    ...
    def _is_combo_port(self, port_id):
        if port_id < 1 or port_id > self.port_count:
            raise ValueError(f"port_id is outside 1..{self.port_count}")
        return port_id in self.parsed_data["combo_port"]

    def configure(self, port_id, **kwargs):
        """
        port_id             port index
        name                port name
        enabled             1 (enable) / 0 (disable), omit to leave unchanged
        autoneg             1 (enable) / 0 (disable), omit to leave unchanged
        duplex              1 (enable) / 0 (disable), omit to leave unchanged
        tx_flow_control     1 (enable) / 0 (disable), omit to leave unchanged
        rx_flow_control     1 (enable) / 0 (disable), omit to leave unchanged
        ...
        # Each of these flags is a set of 1-based port_ids (see utils.decode_listofflags).
        # Omitting the kwarg leaves the current setting untouched, matching the
        # convention used in every other module (e.g. Mikrotik_System.set()).
        for kw, flag_set_name in (
            ("enabled", "enabled"),
            ("autoneg", "autoneg"),
            ("duplex", "duplex"),
            ...
            if value is not None:
                if value:
                    self.parsed_data[flag_set_name].add(port_id)
                else:
                    self.parsed_data[flag_set_name].discard(port_id)
    ...
    def save(self, dry_run=False):
        self._update_data("en", utils.encode_listofflags(self.parsed_data["enabled"], self.port_count))
        self._update_data("dpxc", utils.encode_listofflags(self.parsed_data["duplex"], self.port_count))
        self._update_data("fctc", utils.encode_listofflags(self.parsed_data["tx_flow_control"], self.port_count))
        self._update_data("fctr", utils.encode_listofflags(self.parsed_data["rx_flow_control"], self.port_count))
        self._update_data("an", utils.encode_listofflags(self.parsed_data["autoneg"], self.port_count))
    ...
    def show(self):
        ...
        print("link tab")
        # i is the 1-based port_id throughout this loop.
        for i in range(1, self.port_count + 1):
            properties = [
                f"enabled: {i in self.parsed_data['enabled']}",
                f"full duplex: {i in self.parsed_data['duplex']}",
                f"flow ctrl tx: {i in self.parsed_data['tx_flow_control']}",
                f"flow ctrl tx: {i in self.parsed_data['rx_flow_control']}",
                f"autoneg: {i in self.parsed_data['autoneg']}"
            ]
            if i not in self.parsed_data['autoneg']:
                # 0-based list -> use i-1
                properties.append(f"speed: {port_speed_mb_str[self.parsed_data['speed'][i-1]]}mb/s")

            if self.version >= 2.16:
                # i is already 1-based, don't add another +1
                if self._is_combo_port(i):
                    properties.append(f"combo mode: {combo_mode_str[self.parsed_data['combo_mode'][i-1]]}")
                if self._is_sfp_port(i):
                    properties.append(f"sfp rate: {sfp_rate_str[self.parsed_data['sfp_rate'][i-1]]}")

            print(f"* {self.parsed_data['name'][i-1]} {', '.join(properties)}")
        print("")
