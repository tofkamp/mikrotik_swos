...
class Mikrotik_Port(Swostab):
    ...
    def configure(self, port_id, **kwargs):
        ...
        port_id in self.parsed_data["enabled"] if kwargs.get("enabled", 0) else 0
        port_id in self.parsed_data["autoneg"] if kwargs.get("autoneg", 1) else 0
        port_id in self.parsed_data["duplex"] if kwargs.get("duplex", 1) else 0
        port_id in self.parsed_data["tx_flow_control"] if kwargs.get("tx_flow_control", 0) else 0
        port_id in self.parsed_data["rx_flow_control"] if kwargs.get("rx_flow_control", 0) else 0
