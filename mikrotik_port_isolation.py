...
class Mikrotik_Forwarding(Swostab):
    ...
    def port_isolation(self, port_id, port_list = []):
        ...
        if isinstance(port_list, str) and port_list == 'any':
            acl = set(range(1, self.port_count + 1))
        else:
            acl = set()
            for p in port_list:
        ...
        acl.remove(port_id)    # always isolate yourself port
        self._update_data("fp{}".format(port_id), utils.encode_listofflags(acl,self.port_count))
        return True
    ...
    def port_vlan_config(self, port_id, mode = None, receive_mode = None, default_vlan_id = None, force_vlan_id = None):
        ...
        if force_vlan_id is not None:
            ...
            if force_vlan_id:
                ...
                _fvid.discard(port_id)

            self._update_data("fvid", utils.encode_listofflags(_fvid,self.port_count))
    ...
    def show(self):
        ...
        for i in range(0, self.port_count):
            ...
            print("  force to vlan id: {}".format((i+1) in _fvid))
        print("")
